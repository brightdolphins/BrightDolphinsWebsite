 ...
 * @since 2.1.1 Fix: le libellé du namespace 'Templates' en 'TemplatesLib'
 *                   Déplacement du fichier dans le répsertoire 'classes'
 */
...
class EAC_Menu_Walker extends \Walker_Nav_Menu {
	...
	public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		$output .= $indent . '<li' . $id . $class_names . $value_role . '>';
		...
		foreach ( $atts as $attr => $value ) {
			if ( is_scalar( $value ) && '' !== $value && false !== $value ) {
				$value       = ( 'href' === $attr ) ? esc_url( $value ) . ' role="menuitem"' : esc_attr( $value );
