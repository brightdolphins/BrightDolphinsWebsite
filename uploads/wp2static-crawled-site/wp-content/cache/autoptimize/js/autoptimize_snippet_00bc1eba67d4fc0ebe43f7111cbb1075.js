...
bindEvents(){this.elements.$swiperArrows.on('keydown',this.onDirectionArrowKeydown.bind(this));this.elements.$paginationWrapper.on('keydown','.swiper-pagination-bullet',this.onDirectionArrowKeydown.bind(this));this.elements.$swiperContainer.on('keydown','.swiper-slide',this.onDirectionArrowKeydown.bind(this));this.$element.find(':focusable').on('focus',this.onFocusDisableAutoplay.bind(this));}
unbindEvents(){this.elements.$swiperArrows.off();this.elements.$paginationWrapper.off();this.elements.$swiperContainer.off();this.$element.find(':focusable').off();}
onDirectionArrowKeydown(event){const isRTL=elementorFrontend.config.isRTL,inlineDirectionArrows=['ArrowLeft','ArrowRight'],currentKeydown=event.originalEvent.code,isDirectionInlineKeydown=-1!==inlineDirectionArrows.indexOf(currentKeydown),directionStart=isRTL?'ArrowRight':'ArrowLeft',directionEnd=isRTL?'ArrowLeft':'ArrowRight';if(!isDirectionInlineKeydown){return true;}else if(directionStart===currentKeydown){this.swiper.slidePrev();}else if(directionEnd===currentKeydown){this.swiper.slideNext();}}
...
"../modules/nested-accordion/assets/js/frontend/handlers/nested-accordion.js"));_modules.default.frontend={Document:_document.default,tools:{StretchElement:_stretchElement.default},handlers:{Base:_base.default,StretchedElement:_stretchedElement.default,SwiperBase:_baseSwiper.default,CarouselBase:_baseCarousel.default,NestedTabs:_nestedTabs.default,NestedAccordion:_nestedAccordion.default}};}),"../assets/dev/js/frontend/tools/stretch-element.js":
/*!**********************************************************!*\
  !*** ../assets/dev/js/frontend/tools/stretch-element.js ***!
  \**********************************************************/
((module)=>{"use strict";module.exports=elementorModules.ViewModule.extend({getDefaultSettings(){return{element:null,direction:elementorFrontend.config.is_rtl?'right':'left',selectors:{container:window},considerScrollbar:false};},getDefaultElements(){return{$element:jQuery(this.getSettings('element'))};},stretch(){const settings=this.getSettings();let $container;try{$container=jQuery(settings.selectors.container);}catch(e){}
...
/*! @babel/runtime/helpers/interopRequireDefault */
"../node_modules/@babel/runtime/helpers/interopRequireDefault.js");Object.defineProperty(exports,"__esModule",({value:true}));exports["default"]=void 0;var _base=_interopRequireDefault(__webpack_require__(
/*! elementor/assets/dev/js/frontend/handlers/base */
"../assets/dev/js/frontend/handlers/base.js"));class NestedAccordion extends _base.default{getDefaultSettings(){return{selectors:{accordionContentContainers:'.e-n-accordion > .e-con',accordionItems:'.e-n-accordion-item'}};}
getDefaultElements(){const selectors=this.getSettings('selectors');return{$contentContainers:this.findElement(selectors.accordionContentContainers),$items:this.findElement(selectors.accordionItems)};}
onInit(){super.onInit(...arguments);if(elementorFrontend.isEditMode()){this.interlaceContainers();}}
interlaceContainers(){const{$contentContainers,$items}=this.getDefaultElements();$contentContainers.each((index,element)=>{$items[index].appendChild(element);});}}
...
/*! @babel/runtime/helpers/interopRequireDefault */
"../node_modules/@babel/runtime/helpers/interopRequireDefault.js");Object.defineProperty(exports,"__esModule",({value:true}));exports["default"]=void 0;var _base=_interopRequireDefault(__webpack_require__(
/*! ../../../../../../assets/dev/js/frontend/handlers/base */
"../assets/dev/js/frontend/handlers/base.js"));class NestedTabs extends _base.default{constructor(){super(...arguments);this.resizeListenerNestedTabs=null;}
getTabTitleFilterSelector(tabIndex){return`[data-tab="${tabIndex}"]`;}
...
onTabKeyDown(event){this.onKeydownAvoidUndesiredPageScrolling(event);}
...
getHeadingEvents(){return{mousedown:this.changeScrollStatus.bind(this),mouseup:this.changeScrollStatus.bind(this),mouseleave:this.changeScrollStatus.bind(this),mousemove:this.setHorizontalTabTitleScrollValues.bind(this)};}
...
unbindEvents(){this.elements.$tabTitles.off();this.elements.$headingContainer.off();elementorFrontend.elements.$window.off('resize');elementorFrontend.elements.$window.off('elementor/nested-tabs/activate');}
...
onElementChange(propertyName){if(this.checkSliderPropsToWatch(propertyName)){this.setHorizontalScrollAlignment();}}
checkSliderPropsToWatch(propertyName){return 0===propertyName.indexOf('horizontal_scroll')||0===propertyName.indexOf('tabs_justify_horizontal')||0===propertyName.indexOf('tabs_title_space_between');}
...
tabTitleHasActiveContentContainer(index){const $tabTitleElement=this.elements.$tabTitles.filter(this.getTabTitleFilterSelector(index)),isTabTitleActive=$tabTitleElement[0]?.classList.contains(`${this.getActiveClass()}`),$tabTitleContainerElement=this.elements.$tabContents.filter(this.getTabContentFilterSelector(index));return!!$tabTitleContainerElement&&isTabTitleActive?true:false;}
changeScrollStatus(event){const slider=this.elements.$headingContainer[0];if('mousedown'===event.type){slider.classList.add('e-scroll');slider.dataset.pageX=event.pageX;}else{slider.classList.remove('e-scroll');slider.classList.remove('e-scroll-active');slider.dataset.pageX='';}}
isHorizontalScroll(){const slider=this.elements.$headingContainer[0];return slider.clientWidth<this.getChildrenWidth(slider.children)&&'enable'===this.getHorizontalScrollSetting();}
getChildrenWidth(children){let totalWidth=0;const parentContainer=children[0].parentNode,computedStyles=getComputedStyle(parentContainer),gap=parseFloat(computedStyles.gap)||0;for(let i=0;i<children.length;i++){totalWidth+=children[i].offsetWidth+gap;}
return totalWidth;}
setHorizontalScrollAlignment(){let event=arguments.length>0&&arguments[0]!==undefined?arguments[0]:{};if(!this.elements){return;}
const slider=this.elements.$headingContainer[0];if(this.isHorizontalScroll()){const tabsDirection=this.getTabsDirection();this.initialScrollPosition(slider,tabsDirection);}else{slider.style.setProperty('--n-tabs-heading-justify-content','');}}
getTabsDirection(){const currentDevice=elementorFrontend.getCurrentDeviceMode(),tabsDirection=elementorFrontend.utils.controls.getResponsiveControlValue(this.getElementSettings(),'tabs_justify_horizontal','',currentDevice);return tabsDirection;}
initialScrollPosition(slider,tabsDirection){const isRTL=elementorCommon.config.isRTL;switch(tabsDirection){case'end':slider.style.setProperty('--n-tabs-heading-justify-content','start');slider.scrollLeft=isRTL?-1*this.getChildrenWidth(slider.children):this.getChildrenWidth(slider.children);break;default:slider.style.setProperty('--n-tabs-heading-justify-content','start');slider.scrollLeft=0;}}
getHorizontalScrollSetting(){const currentDevice=elementorFrontend.getCurrentDeviceMode(),horizontalScrollSetting=elementorFrontend.utils.controls.getResponsiveControlValue(this.getElementSettings(),'horizontal_scroll','',currentDevice);return horizontalScrollSetting;}
setHorizontalTabTitleScrollValues(event){const slider=this.elements.$headingContainer[0],isActiveScroll=slider.classList.contains('e-scroll'),isHorizontalScrollActive='enable'===this.getHorizontalScrollSetting(),headingContentIsWiderThanWrapper=slider.scrollWidth>slider.clientWidth;if(!isActiveScroll||!isHorizontalScrollActive||!headingContentIsWiderThanWrapper){return;}
event.preventDefault();const previousPositionX=parseFloat(slider.dataset.pageX),mouseMoveX=event.pageX-previousPositionX,maximumScrollValue=5,stepLimit=20;let toScrollDistanceX=0;if(stepLimit<mouseMoveX){toScrollDistanceX=maximumScrollValue;}else if(stepLimit*-1>mouseMoveX){toScrollDistanceX=-1*maximumScrollValue;}else{toScrollDistanceX=mouseMoveX;}
slider.scrollLeft=slider.scrollLeft-toScrollDistanceX;slider.classList.add('e-scroll-active');}}
...
/*!****************************************************************!*\
  ...
  \****************************************************************/
((module,__unused_webpack_exports,__webpack_require__)=>{var createNonEnumerableProperty=__webpack_require__(
