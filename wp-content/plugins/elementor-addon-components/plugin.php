...
class EAC_Plugin {
	...
	public function __construct() {
		...
		require_once(__DIR__ . '/includes/eac-load-config.php');
		 ...
		 */
		require_once(__DIR__ . '/includes/eac-load-features.php');
		
		/** @since 1.9.2 Charge les scripts et les styles globaux */
		require_once(__DIR__ . '/includes/eac-load-scripts.php');
		
		/** @since 1.9.8 Charge les catégories, les controls et les composants Elementor */
		require_once(__DIR__ . '/includes/eac-load-elements.php');
	}
	...
	public function set_grant_option_page() {
		...
		/** ACF V5 && ACF Options Page active && Grant Options Page active */
		...
		if($grant_option_page) {
			...
			if($role->has_cap($this->manage_options) === false) {
				wp_roles()->add_cap('editor', $this->manage_options);
			}
			
			$role = get_role('shop_manager');
			if(!is_null($role) && $role->has_cap($this->manage_options) === false) {
				wp_roles()->add_cap('shop_manager', $this->manage_options);
			}
		} else {
			$role = get_role('editor');
			if($role->has_cap($this->manage_options) === true) {
				wp_roles()->remove_cap('editor', $this->manage_options);
			}
			
			$role = get_role('shop_manager');
			if(!is_null($role) && $role->has_cap($this->manage_options) === true) {
