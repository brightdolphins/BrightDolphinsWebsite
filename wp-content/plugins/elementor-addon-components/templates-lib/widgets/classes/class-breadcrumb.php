 ...
 *
 * Description:
 * IQ Balrony => https://gist.github.com/iqbalrony/55ae1737aed969809ad06e3b8f0525b1
 * Code inspiration from this plugin:- https://github.com/justintadlock/breadcrumb-trail
 *
 * @since 2.1.1
 */

namespace EACCustomWidgets\TemplatesLib\Widgets\Classes;
...
class Breadcrumb_Trail {
	 ...
	 *     @type string    $home_icon        Home icon before home title.
	 *     @type string    $home_icon_class  Class attributes to use in the home icon wrapper.
	...
	public function __construct( $args = array() ) {
		...
		$defaults = array(
			'list_tag'        => 'span',
			'list_class'      => 'eac-breadcrumbs-items',
			'item_tag'        => 'span',
			'item_class'      => 'eac-breadcrumbs-item',
			'separator'       => ' \ ',
			'separator_class' => 'eac-breadcrumbs-separator',
			'home_icon'       => '',
			'home_icon_class' => 'eac-breadcrumbs-home-icon',
			'show_on_front'   => true,
			'network'         => false,
			'show_title'      => true,
			'trunk_title'     => 0,
	...
	public function trail() {
		...
		if ( 0 < $item_count ) {
			...
			$list_class = ! empty( $this->args['list_class'] ) ? ' class="' . esc_attr( $this->args['list_class'] ) . '"' : '';
			...
			foreach ( $this->items as $item ) {
				...
				if ( 1 === $item_position && 1 < $item_count ) {
					$item_class .= ' eac-breadcrumbs-start';
				} elseif ( $item_count === $item_position ) {
					$item_class .= ' eac-breadcrumbs-end';
					/** @since 2.1.1 wp_trim_words fait un wp_strip_all_tags donc pas de souci */
					if ( true === $this->args['show_title'] && 0 < $this->args['trunk_title'] ) {
						$item = wp_trim_words( $item, $this->args['trunk_title'], '...' );
					}
				}
	...
	public function set_separator( $separator = '', $separator_class = '' ) {
		...
		$separator  = sprintf( '<%1$s %2$s>%3$s</%1$s>', tag_escape( 'span' ), $attributes, $separator );
		return $separator;
	}
	...
	protected function set_labels() {
		...
		$defaults = array(
			...
			'archive_week'        => esc_html__( 'Semaine %s', 'eac-components' ),
	...
	protected function add_blog_items() {
		...
		if ( is_null( $post ) ) {
			// error_log( 'add_blog_items Post est NULL' );
			return;
		}
	...
	protected function add_singular_items() {
		...
		if ( is_null( $post ) ) {
			// error_log( 'add_singular_items Post est NULL' );
			return;
		}
		$post_id = get_queried_object_id();
	...
	protected function add_term_archive_items() {
		...
		if ( ! $taxonomy ) {
			// error_log( 'add_term_archive_items est NULL' );
			return;
		}
		...
		if ( false === $done_post_type && 1 === count( $taxonomy->object_type ) && post_type_exists( $taxonomy->object_type[0] ) ) {
			...
			if ( 'post' === $taxonomy->object_type[0] ) {
				 ...
				 *
				 * @since 2.1.1 Change l'URL de de la catégorie/étiquette si l'option 'metas' est activée
				 */
			} else {
				$metas            = false;
				$redirect_url     = '';
				...
				if ( $metas && '' !== $redirect_url ) {
					$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( $redirect_url ), $label );
				} else {
					$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_post_type_archive_link( $post_type_object->name ) ), $label );
				}
			}
	...
	protected function add_minute_hour_archive_items() {
		...
		if ( true === $this->args['show_title'] ) {
			$this->items[] = sprintf( $this->labels['archive_minute_hour'], get_the_time( esc_html_x( 'g:i a', 'minute and hour archives time format', 'eac-components' ) ) );
		}
	}
	...
	protected function add_minute_archive_items() {
		...
		// Add the minute item.
		if ( true === $this->args['show_title'] ) {
			$this->items[] = sprintf( $this->labels['archive_minute'], get_the_time( esc_html_x( 'i', 'minute archives time format', 'eac-components' ) ) );
		}
	}
	...
	protected function add_hour_archive_items() {
		...
		// Add the hour item.
		if ( true === $this->args['show_title'] ) {
			$this->items[] = sprintf( $this->labels['archive_hour'], get_the_time( esc_html_x( 'g a', 'hour archives time format', 'eac-components' ) ) );
		}
	}
	...
	protected function add_day_archive_items() {
		...
		$year  = sprintf( $this->labels['archive_year'], get_the_time( esc_html_x( 'Y', 'yearly archives date format', 'eac-components' ) ) );
		$month = sprintf( $this->labels['archive_month'], get_the_time( esc_html_x( 'F', 'monthly archives date format', 'eac-components' ) ) );
		$day   = sprintf( $this->labels['archive_day'], get_the_time( esc_html_x( 'j', 'daily archives date format', 'eac-components' ) ) );
	...
	protected function add_week_archive_items() {
		...
		$year = sprintf( $this->labels['archive_year'], get_the_time( esc_html_x( 'Y', 'yearly archives date format', 'eac-components' ) ) );
		$week = sprintf( $this->labels['archive_week'], get_the_time( esc_html_x( 'W', 'weekly archives date format', 'eac-components' ) ) );
	...
	protected function add_post_hierarchy( $post_id ) {
		 ...
		 *
		 * @since 2.1.1 Change l'URL de 'Single product page' si l'option 'breadcrumb' est activée
		 */
	...
	protected function map_rewrite_tags( $post_id, $path ) {
		...
		if ( is_array( $matches ) ) {
			...
			foreach ( $matches as $match ) {
				...
				if ( '%year%' === $tag ) {
					$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_year_link( get_the_time( 'Y', $post_id ) ) ), sprintf( $this->labels['archive_year'], get_the_time( esc_html_x( 'Y', 'yearly archives date format', 'eac-components' ) ) ) );
				} elseif ( '%monthnum%' === $tag ) { // If using the %monthnum% tag, add a link to the monthly archive.
					$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_month_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ) ) ), sprintf( $this->labels['archive_month'], get_the_time( esc_html_x( 'F', 'monthly archives date format', 'eac-components' ) ) ) );
				} elseif ( '%day%' === $tag ) { // If using the %day% tag, add a link to the daily archive.
					$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_day_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ), get_the_time( 'd', $post_id ) ) ), sprintf( $this->labels['archive_day'], get_the_time( esc_html_x( 'j', 'daily archives date format', 'eac-components' ) ) ) );
